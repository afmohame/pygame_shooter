...
radius, orbit_rad, center = 10, 50, (400, 400)
width, height = 80, 120
weapon_x, weapon_y = 50, 60
revolver = Weapons(weapon_x, weapon_y, 3, 10, "common", sprite_revolver, radius, 2, (center[0], center[1]))
...
while run:
    ...
    dx, dy = revolver.mx - center[0], revolver.my - center[1]
    angle = math.degrees(math.atan2(dy, dx))-corr_angle
    
    revolver.Rotate_gun(angle, orbit_rad, 0, center[0], center[1])

    #character hitbox
    screen.blit(surface_char, (center[0] - width//2, center[1] - height//2))
    screen.blit(anchor_surface, (center[0] - radius/2, center[1] - radius/2))
    screen.blit(revolver.gun_surf, revolver.rot_gun_screen)
